#!/usr/bin/env python3
"""Pre-bin the EMPIAR-10017 micrographs by 4x and scale the known
particles.star coords by 1/4. The result is a self-contained dataset at
data/projects/empiar10017_bin4/ that the import task can pick up directly
(no on-the-fly binning needed)."""
import os, sys, shutil
import numpy as np
import mrcfile

SRC = "/home/z/my-project/data/projects/empiar10017"
DST = "/home/z/my-project/data/projects/empiar10017_bin4"
BIN = 4
ANGPIX_ORIG = 1.77
ANGPIX_EFF = ANGPIX_ORIG * BIN  # 7.08
...
print(f"[bin4] {len(src_mics)} micrographs to bin (4096 -> 1024, angpix {ANGPIX_ORIG} -> {ANGPIX_EFF})")
for i, name in enumerate(src_mics):
    out = os.path.join(DST, "Micrographs", name)
    if os.path.exists(out):
        print(f"[bin4] {i+1}/{len(src_mics)} {name} (cached)")
        continue
...
with open(src_star) as fin, open(dst_star, "w") as fout:
    ...
    for line in fin:
        ...
        try:
            ...
            fout.write(line)
print(f"[bin4] scaled {n_scaled} coords by 1/{BIN} -> {dst_star}")
print(f"[bin4] DONE: dataset at {DST}")
