...
def resolve_source(src):
    """Resolve a source_dataset path to absolute. Relative paths are joined
    to PROJECT_ROOT (so 'data/projects/test_d4' works regardless of the
    runner's CWD)."""
    if not src:
        return os.path.join(PROJECT_ROOT, "data", "projects", "test_d4")
    if os.path.isabs(src):
        return src
    return os.path.normpath(os.path.join(PROJECT_ROOT, src))
